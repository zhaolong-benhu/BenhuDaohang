...
export default class GameNews extends React.Component{
    ...
    constructor(){
        super();
        superagent.get(game_news_json).end(function(err,res){
            if(!err){
    ...
    handleSubmit(e){
        e.preventDefault();
        superagent.post(game_news_json).set('Content-Type', 'application/json')
    ...
    handleLinkChange(index,value_index,group,type,e){
        ...
        if (index == 0){
          ...
          data.game_categories[index][group] = data[group];
        }else{
          data.game_categories[index][group][value_index][type] = e.target.value;
        }
    ...
    render(){
        return(
            <form  className="game-news" onSubmit={this.handleSubmit.bind(this)}>
              <div className="value" >
