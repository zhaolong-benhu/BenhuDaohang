...
export default class LifeNews extends React.Component{
    ...
    constructor(){
        super();
        superagent.get(life_news_json).end(function(err,res){
            if(!err){
    ...
    handleSubmit(e){
        e.preventDefault();
        superagent.post(life_news_json).set('Content-Type', 'application/json')
    ...
    handleLinkChange(index,value_index,group,type,e){
        ...
        if (index == 0){
          ...
          data.life_categories[index][group] = data[group];
        }else{
          data.life_categories[index][group][value_index][type] = e.target.value;
        }
    ...
    {
        let data = this.state.modules;
        data.life_categories[index].name = e.target.value;
        this.setState({modules:data});
    }
    render(){
        return(
            <form  className="life-news" onSubmit={this.handleSubmit.bind(this)}>
              <div className="value" >
