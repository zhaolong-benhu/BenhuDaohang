...
export default class GameSection extends React.Component{
    ...
    constructor(){
        super();
        superagent.get(game_section_json).end(function(err,res){
            if(!err){
    ...
    handleSubmit(e){
        e.preventDefault();
        superagent.post(game_section_json).set('Content-Type', 'application/json')
    ...
    handleRigthLinkChange(index,value_index,type,e){
        let data = this.state.modules;
        if (index == 0){
          data.right_game_items[value_index][type] = e.target.value;
          data.game_categories[index].right_game_items = data.right_game_items;
        }else{
          data.game_categories[index]["right_game_items"][value_index][type] = e.target.value;
        }
        this.setState({modules:data});
    }
    ...
    {
        let data = this.state.modules;
        data.game_categories[index].name = e.target.value;
        this.setState({modules:data});
    }
    render(){
        return(
            <form  className="game-section" onSubmit={this.handleSubmit.bind(this)}>
              <div className="value" >
                  <div className="link">
                  ...
                  {this.state.modules.game_categories.map(function(value,index){
                    return <div className="right-items" key={index}>
                               ...
                               {value.right_game_items.map(function(data,value_index){
                                 return <div className="other-div link" key={value_index}>
                                           <input type="text" value={data.title} onChange={this.handleRigthLinkChange.bind(this,index,value_index,'title')} placeholder="游戏名称"/>
                                           <input type="text" value={data.ad_title} onChange={this.handleRigthLinkChange.bind(this,index,value_index,'ad_title')} placeholder="标题"/>
