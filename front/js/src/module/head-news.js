...
export default class HeadNews extends React.Component{
    ...
    constructor(){
        super();
        superagent.get(head_news_json).set('Accept', 'application/json').end(function(err,res){
            if(!err){
    ...
    head_categories_click(index){
        var data = this.state.modules;
        data.current_active = index;
        data.left_img_items = this.state.modules.head_categories[index].left_img_items;
        data.left_char_items = this.state.modules.head_categories[index].left_char_items;
            ...
            <div className="head-news">
                <div className="header">
                  {this.state.modules.head_categories.map(function(value,index){
                      ...
                      return <a href="javascript:void(0)" className={active} key={'head_categories' + index } onClick={this.head_categories_click.bind(this,index)}>
